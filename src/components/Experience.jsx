    ...
    return (
        ...
        >
            <div>
                ...
                <p className='text-white text-base font-semibold' style={{ margin: 0 }}>
                    {experience.company_name}
                </p>
                <p className='text-white text-base font-semibold'>{experience.headline}</p>
            </div>
            <ul className='mt-5 list-disc ml-5 space-y-2'>
                {experience.points.map((point, index) => (
                    <li
                        key={`experience-point-${index}`}
                        className='text-secondary-100 text-sm pl-1 tracking-wider'
                    >
                        {point}
                    </li>
                ))}
